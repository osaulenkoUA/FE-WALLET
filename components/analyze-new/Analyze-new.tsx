...
export const AnalyzeNew = () => {
	...
	const monthlySpendingCard = () => {
		return items
			.filter((el) => el.paymentMethod === "card")
			.reduce((acc, el) => {
				return acc + el.amount;
			}, 0);
	};
	const monthlySpendingCash = () => {
		return items
			.filter((el) => el.paymentMethod === "cash")
			.reduce((acc, el) => {
				return acc + el.amount;
			}, 0);
	};
	...
	const getSummaryCategory = (category: string) => {
		return items
			.filter((el) => el.category.name === category)
			.reduce((acc, el) => {
				return acc + el.amount;
			}, 0);
	};
	...
	const getColor = (el: any) => {
		const listDates = items
			.filter((f) => f.category.name === el.name)
			.map((m) => m.create_date.split("T")[0]);
