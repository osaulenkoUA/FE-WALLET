...
export default function Addcategory() {
    ...
    return (
            <div className={'flex flex-col'}>
                ...
                <button disabled={!cat} onClick={onHandleSubmit} className={'border-2 w-max pl-8 pr-8 m-auto'}>Добавити
                </button>
                <div>
                    {financesStore.category.map(el => (<p className={'mt-2 text-xl text-blue-900'} key={el.id}>
                        {el.name}
                    </p>))}
                </div>
            </div>
