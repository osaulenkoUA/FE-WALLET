...
export const Analyze = () => {
		...
		try {
			...
			if (data.status === 202) {
				...
				const newItems = items.map((el) =>
					el._id === data.data._id ? data.data : el,
				);
				setItems(newItems);
			}
		} catch (err) {
	...
	const getColor = (el: any) => {
		const listDates = items
			.filter((f) => f.category === el.name)
			.map((m) => m.date);
		const currDate = new Date().getDate();
		const color = listDates.some(
			(d) =>
				d.slice(0, 2) === (currDate > 10 ? currDate.toFixed() : "0" + currDate),
		);
		...
		<div>
			{!loading && (
				<div className={"p-[16px]"}>
					<div
						className={"grid grid-cols-3 gap-4 items-center justify-end mb-2"}
					>
						...
						<div
							className={"cursor-pointer w-[36px] h-[36px] justify-self-end"}
							onClick={() => {
					...
					<div className={"grid grid-cols-1 lg:grid-cols-3"}>
						{categoryies.map((c) => (
							<div className={"mb-2"} key={c.id}>
								<div
									className={
										"flex bg-gradient-to-r from-blue-500 to-teal-500 rounded-2xl items-center shadow-lg hover:shadow-xl transition-shadow duration-300"
									}
								>
									<p
										onClick={() => {
										...
										id={`category-${c.name}`}
										className={
											"text-gray-100 w-full p-3 text-[18px] font-bold hover:text-white transition-colors duration-300 cursor-pointer"
										}
									>
										{c?.name}
									</p>
									<p
										className={`p-3 text-[18px] font-bold bg-opacity-20 rounded-lg ${getColor(c)}`}
									>
										{getSummaryCategory(c.name)}
									</p>
								</div>

								{category === c.name && (
									<div>
										{getItemsByCategory(c.name)
											.reverse()
											.map((el) => (
												<div
													className={
														"grid-cols-col3 grid items-center gap-4 mb-1"
													}
													key={el._id}
												>
					...
					<div className="space-y-2 bg-white p-6 rounded-lg shadow-md">
						{/* Total Amount */}
						<div className="flex justify-between items-center bg-gradient-to-r from-purple-600 to-indigo-600 p-4 rounded-lg">
						...
						{/* Card Amount */}
						<div className="flex justify-between items-center bg-gray-100 p-4 rounded-lg hover:bg-gray-200 transition-colors duration-300">
							<p className="text-gray-700 text-xl font-medium">Сума картка</p>
							<p className="text-gray-700 text-xl font-semibold">
								{monthlySpendingCard()} грн
							</p>
						</div>
